...
class MultiHeadedSelfAttention(nn.Module):
    ...
    def __init__(self, embedding_dim: int, attention_dim: int, num_heads: int):
        ...
        self.attention_heads = nn.ModuleList([self.SingleHeadAttention(embedding_dim, attention_dim // num_heads) for _ in range(num_heads)])
    ...
    def forward(self, embedded: TensorType[float]) -> TensorType[float]:
        ...
        attention_head_outputs = torch.cat([attention_head(embedded) for attention_head in self.attention_heads], dim=-1)
        # Pass concatenated result through the output projection (W_O)
        projection_output = self.projection(attention_head_outputs)
        # Return result rounded to 4 decimal places
        return torch.round(projection_output, decimals=4)
